...
func SetupHandler(deps deliveryHttpDeps.Deps) http.Handler {
	...
	api.Handle(http.MethodGet, "/clients", jwtAuth(dashboardH.List))
	api.Handle(http.MethodGet, "/clients/{company_id}", jwtAuth(dashboardH.Get))
	api.Handle(http.MethodPost, "/clients", jwtAuth(dashboardH.Create))
	api.Handle(http.MethodPut, "/clients/{company_id}", jwtAuth(dashboardH.Update))
	api.Handle(http.MethodDelete, "/clients/{company_id}", jwtAuth(dashboardH.Delete))
